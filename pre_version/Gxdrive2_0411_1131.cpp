...
void Gxdrive::depth_map()
{
    while (ros::ok())
    {
        if (this->frame > this->sync_frame + 4 && left_gray != NULL && right_gray != NULL)
        {
            ...
            // plane_left.data = this->left_gray;
            // std::cout << "success" << std::endl;
            // if (vpiimage_left == NULL || vpiimage_right == NULL)
            // {
            //     CHECK_STATUS(vpiImageCreateWrapperOpenCVMat(vpileft, 0, &vpiimage_left));
            //     CHECK_STATUS(vpiImageCreateWrapperOpenCVMat(vpiright, 0, &vpiimage_right));
            // }
            // else
            // {
            //     CHECK_STATUS(vpiImageSetWrappedOpenCVMat(vpiimage_left, vpileft));
            //     CHECK_STATUS(vpiImageSetWrappedOpenCVMat(vpiimage_right, vpiright));
            // }
            ...
            if (vpiimage_left == NULL || vpiimage_right == NULL)
            {
                ...
                CHECK_STATUS(vpiImageCreateWrapper(&vpiData_right, nullptr, 0, &tmpLeft));
            }
            else
            {
            ...
            VPIImageData data;
            CHECK_STATUS(vpiImageLockData(tmpLeft, VPI_LOCK_READ, VPI_IMAGE_BUFFER_HOST_PITCH_LINEAR, &data));
            VPIImageBufferPitchLinear &outPitch = data.buffer.pitch;
            ...
            CHECK_STATUS(vpiImageUnlock(tmpLeft));
