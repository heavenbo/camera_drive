...
void Gxdrive::Init()
{
    ...
    this->stereo_disparity.encoding = "mono16";
    this->stereo_disparity.is_bigendian = false;
    this->stereo_disparity.step = this->width * 2;
    // this->stereo_disparity.step = this->width ;
    this->stereo_disparity.data.resize(this->stereo_disparity.step * this->height);
    // vpi right data
    memset(&vpiData_right, 0, sizeof(vpiData_right));
    vpiData_right.bufferType = VPI_IMAGE_BUFFER_HOST_PITCH_LINEAR;
    vpiData_right.buffer.pitch.format = VPI_IMAGE_FORMAT_U8;
    vpiData_right.buffer.pitch.numPlanes = 1;
    // VPIImagePlanePitchLinear &plane_right = vpiData_right.buffer.pitch.planes[0];
    vpiData_right.buffer.pitch.planes[0].width = this->width;
    vpiData_right.buffer.pitch.planes[0].height = this->height;
    vpiData_right.buffer.pitch.planes[0].pitchBytes = this->width;
    vpiData_right.buffer.pitch.planes[0].pixelType = vpiImageFormatGetPlanePixelType(VPI_IMAGE_FORMAT_U8, 0);
    // vpi left data
    memset(&vpiData_left, 0, sizeof(vpiData_left));
    vpiData_left.bufferType = VPI_IMAGE_BUFFER_HOST_PITCH_LINEAR;
    vpiData_left.buffer.pitch.format = VPI_IMAGE_FORMAT_U8;
    vpiData_left.buffer.pitch.numPlanes = 1;
    // VPIImagePlanePitchLinear &plane_left = vpiData_left.buffer.pitch.planes[0];
    vpiData_left.buffer.pitch.planes[0].width = this->width;
    vpiData_left.buffer.pitch.planes[0].height = this->height;
    vpiData_left.buffer.pitch.planes[0].pitchBytes = this->width;
    vpiData_left.buffer.pitch.planes[0].pixelType = vpiImageFormatGetPlanePixelType(VPI_IMAGE_FORMAT_U8, 0);

    memset(&vpidata16, 0, sizeof(vpidata16));
    vpidata16.bufferType = VPI_IMAGE_BUFFER_HOST_PITCH_LINEAR;
    vpidata16.buffer.pitch.format = VPI_IMAGE_FORMAT_U8;
    vpidata16.buffer.pitch.numPlanes = 1;
    // VPIImagePlanePitchLinear &plane_left = vpiData_left.buffer.pitch.planes[0];
    vpidata16.buffer.pitch.planes[0].width = this->width;
    vpidata16.buffer.pitch.planes[0].height = this->height;
    vpidata16.buffer.pitch.planes[0].pitchBytes = this->width;
    vpidata16.buffer.pitch.planes[0].pixelType = vpiImageFormatGetPlanePixelType(VPI_IMAGE_FORMAT_U8, 0);
    ...
    while (ros::ok())
    {
        ...
        if (status == GX_STATUS_SUCCESS)
        {
            ...
            if (this->g_frame_data.nStatus == 0)
            {
                ...
                memcpy((u_char *)vpiData_left.buffer.pitch.planes[0].data, this->left_gray, nWidth * nHeight);
                memcpy((u_char *)vpiData_right.buffer.pitch.planes[0].data, this->right_gray, nWidth * nHeight);
...
void Gxdrive::depth_map()
{
    while (ros::ok())
    {
        if (this->frame > this->sync_frame + 4 && left_gray != NULL && right_gray != NULL)
        {
            ...
            if (vpiimage_left == NULL || vpiimage_right == NULL)
            {
                std::cout << "success" << std::endl;
                CHECK_STATUS(vpiInitConvertImageFormatParams(&convParams));
                CHECK_STATUS(vpiInitConvertImageFormatParams(&convParams_U8));
                ...
                CHECK_STATUS(vpiImageCreateWrapper(&vpiData_left, nullptr, 0, &vpiimage_left));
                CHECK_STATUS(vpiImageCreateWrapper(&vpiData_right, nullptr, 0, &vpiimage_right));
                CHECK_STATUS(vpiImageCreateWrapper(&vpiData_right, nullptr, 0, &confidenceMap_u8));
                ROS_INFO("11");
                vpidata16.buffer.pitch.format = VPI_IMAGE_FORMAT_S16;
                vpidata16.buffer.pitch.planes[0].pitchBytes = this->width * 2;
                vpidata16.buffer.pitch.planes[0].pixelType = vpiImageFormatGetPlanePixelType(VPI_IMAGE_FORMAT_S16, 0);
                CHECK_STATUS(vpiImageCreateWrapper(&vpidata16, nullptr, 0, &disparity));
                // CHECK_STATUS(vpiImageCreate(width, height, VPI_IMAGE_FORMAT_S16, 0, &disparity));
                ROS_INFO("12");
                vpidata16.buffer.pitch.format = VPI_IMAGE_FORMAT_U16;
                vpidata16.buffer.pitch.planes[0].pitchBytes = this->width * 2;
                vpidata16.buffer.pitch.planes[0].pixelType = vpiImageFormatGetPlanePixelType(VPI_IMAGE_FORMAT_U16, 0);
                CHECK_STATUS(vpiImageCreateWrapper(&vpidata16, nullptr, 0, &confidenceMap));
                // CHECK_STATUS(vpiImageCreate(outputWidth, outputHeight, VPI_IMAGE_FORMAT_U16, 0, &confidenceMap));
                ROS_INFO("13");
                vpidata16.buffer.pitch.format = VPI_IMAGE_FORMAT_U16;
                vpidata16.buffer.pitch.planes[0].pitchBytes = this->width * 2;
                vpidata16.buffer.pitch.planes[0].pixelType = vpiImageFormatGetPlanePixelType(VPI_IMAGE_FORMAT_Y16_ER, 0);
                CHECK_STATUS(vpiImageCreateWrapper(&vpidata16, nullptr, 0, &stereoLeft));
                CHECK_STATUS(vpiImageCreateWrapper(&vpidata16, nullptr, 0, &stereoRight));
                // Create the input stereo images
                // CHECK_STATUS(vpiImageCreate(width, height, VPI_IMAGE_FORMAT_Y16_ER, 0, &stereoLeft));
                // CHECK_STATUS(vpiImageCreate(width, height, VPI_IMAGE_FORMAT_Y16_ER, 0, &stereoRight));
                CHECK_STATUS(vpiCreateStereoDisparityEstimator(VPI_BACKEND_CUDA, width, height, VPI_IMAGE_FORMAT_U16, &stereoParams,
                                                               &stereo));
                params.windowSize = 5;
            ...
            {
                CHECK_STATUS(vpiImageSetWrapper(vpiimage_left, &vpiData_left));
                CHECK_STATUS(vpiImageSetWrapper(vpiimage_right, &vpiData_right));
            }
            ROS_INFO("1");
            CHECK_STATUS(vpiSubmitConvertImageFormat(stream, VPI_BACKEND_CPU, vpiimage_left, stereoLeft, &convParams));
            CHECK_STATUS(vpiSubmitConvertImageFormat(stream, VPI_BACKEND_CPU, vpiimage_right, stereoRight, &convParams));
            ROS_INFO("2");
            CHECK_STATUS(vpiSubmitStereoDisparityEstimator(stream, VPI_BACKEND_CUDA, stereo, stereoLeft, stereoRight, disparity,
                                                           confidenceMap, &params));
            // ROS_INFO("3");
            // CHECK_STATUS(vpiSubmitConvertImageFormat(stream, VPI_BACKEND_CPU, confidenceMap, confidenceMap_u8, &convParams_U8));
            ROS_INFO("4");
            CHECK_STATUS(vpiStreamSync(stream));
            ROS_INFO("5");
            // CHECK_STATUS(vpiSubmitConvertImageFormat(stream, VPI_BACKEND_CPU, vpiimage_left, tmpLeft, &cvtParams));
            // CHECK_STATUS(vpiStreamSync(stream));
            VPIImageData data;
            ...
            memcpy(reinterpret_cast<u_char *>(&this->stereo_disparity.data[0]), (u_char *)data.buffer.pitch.planes[0].data, width * height * 2);
