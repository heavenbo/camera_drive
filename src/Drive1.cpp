...
ros::Time current_time = ros::Time(0, 0); // 传输来的时间戳
ros::Time last_time = ros::Time(0, 0);    // 上一帧赋给图片的时间戳
ros::Time time_image;                     // 实际赋给图片的时间戳
...
void Time_callback(const mavros_msgs::CamIMUStampConstPtr &cam_stamp)
{
    ...
    if (isfirst)
    {
        ROS_INFO("First id:%d", cam_stamp->frame_seq_id);
    ...
    else
    {
        ...
        if ((last_tmp_inter - (time_q.front() - current_time)).toSec() > 0.001 || (last_tmp_inter - (time_q.front() - current_time)).toSec() < -0.001)
        {
            ROS_ERROR("id:%d,inter too large:%lf,%lf", cam_stamp->frame_seq_id, last_tmp_inter.toSec(), (time_q.front() - current_time).toSec());
            ROS_ERROR("timestamp:%lf,last_time:%lf", time_q.front().toSec(), current_time.toSec());
        }
        last_tmp_inter = time_q.front() - current_time;
        if (last_tmp_inter.toSec() < -0.001)
        {
            ROS_ERROR("time error,the past ahead of the present");
            ROS_ERROR("current:%lf,last:%lf", time_q.front().toSec(), current_time.toSec());
        }
    ...
    while (ros::ok())
    {
        ...
        if (status == GX_STATUS_SUCCESS)
        {
            if (g_frame_data.nStatus == 0)
            {
                ...
                // ROS_INFO("image: id:%d", id_front);
                if (current_time.toSec() > 0.5)
                {
                    // ROS_INFO("time_image_:%lf", time_image.toSec());
                    time_image = current_time - ros::Duration(0, exposure_t * 500);
                    // ROS_INFO("time_image_1:%lf", time_image.toSec());
                    if (last_time.toNSec() != 0 && (fabs((time_image - last_time).toSec()) - pwm_period_s) >= 0.001)
                    {
                        ROS_WARN("id:%d,image: Timestamp exception,last_time:%lf,current_time:%lf", id_front, last_time.toSec(), time_image.toSec());
                        time_image = time_image + duration_min;
                    }
                    img_left_gray_p = &photo_left_q.front();
                    img_right_gray_p = &photo_right_q.front();
                    // 左图灰度图
                    img_left_gray_p->header.stamp = time_image;
                    img_left_gray_p->header.seq = id_image_;
                    // 右图灰度图
                    img_right_gray_p->header.stamp = time_image;
                    img_right_gray_p->header.seq = id_image_;
                    // cam_left_info
                    cam_left.header.stamp = time_image;
                    cam_left.header.seq = id_image_;
                    // cam_right_info
                    cam_right.header.stamp = time_image;
                    ...
                    // 记录图片数和上一次的时间戳
                    id_image_++;
                    last_time = time_image;
                    // ROS_INFO("last_time:%lf", last_time.toSec());
                    // ROS_INFO("***************");
                }
            }
            // 左图
            img_left.header.stamp = time_image;
            // 右图
            img_right.header.stamp = time_image;
