        ...
        if (status == GX_STATUS_SUCCESS)
        {
            ...
            if (this->g_frame_data.nStatus == 0)
            {
                ...
                if (this->frame >= this->sync_frame)
                {
                    ...
                    this->gframe_img_dura = this->time_image.toNSec() - this->g_frame_data.nTimestamp;
                    // ROS_INFO("fpga:%lf", ros::Time::now().toSec() - this->g_frame_data.nTimestamp / 1000000000.0);
                    LOG(INFO) << std::setprecision(20) << (this->current_time.toNSec() - ros::Time::now().toNSec() + img_ros_dura_first) / 1000000000.0 << '\t'
                              << std::setprecision(20) << (this->g_frame_data.nTimestamp - ros::Time::now().toNSec() + gframe_ros_dura_first) / 1000000000.0 << '\t'
                              << std::setprecision(20) << this->current_time.toSec() - this->g_frame_data.nTimestamp / 1000000000.0 - this->gframe_img_dura_first / 1000000000.0;
                    if (this->gframe_img_dura_min == 0)
                    {

                        this->gframe_img_dura_min = this->gframe_img_dura;
                        this->gframe_img_dura_first = this->current_time.toNSec() - this->g_frame_data.nTimestamp;
                        this->gframe_ros_dura_first = ros::Time::now().toNSec() - this->g_frame_data.nTimestamp;
                        this->img_ros_dura_first = ros::Time::now().toNSec() - this->current_time.toNSec();
                    }
                    else if (this->gframe_img_dura_min > this->gframe_img_dura && this->gframe_img_dura_min - this->gframe_img_dura < 1000000)
                    {
                        if (this->time_image.toSec() - this->last_time_image.toSec() > this->pwm_period_s)
                        {
                            this->gframe_img_dura_min = this->gframe_img_dura;
                        }
                    }
                    // ROS_INFO("fpga_gx_dura_min:%ld", fpga_gx_dura_min);
                    if (this->gframe_img_dura - this->gframe_img_dura_min > 3000000)
                    {
                        ...
                        uint64_t true_time = this->g_frame_data.nTimestamp + this->gframe_img_dura_min;
                        this->time_image = ros::Time((uint32_t)(true_time / 1000000000), true_time % 1000000000);
