...
void Gxdrive::yaml_read(std::string file_path)
{
    ...
    this->is_undistort = yaml_node["Is_undistort"].as<bool>();
...
void Gxdrive::remap_Init()
{
    ...
    cv::initUndistortRectifyMap(K_0, Dist_0, cv::Mat_<double>::eye(3, 3), K_0.clone(), cv::Size(width, height), CV_32FC1, mapLx, mapLy);
    cv::initUndistortRectifyMap(K_1, Dist_1, cv::Mat_<double>::eye(3, 3), K_1.clone(), cv::Size(width, height), CV_32FC1, mapRx, mapRy);
    ...
    while (ros::ok())
    {
        ...
        if (status == GX_STATUS_SUCCESS)
        {
            ...
            if (this->g_frame_data.nStatus == 0)
            {
                ...
                if (is_undistort)
                {
                    nppiRemap_8u_C1R(this->left_gray, {nWidth, nHeight}, nWidth, {0, 0, nWidth, nHeight}, pLxmap, 3840, pLymap, 3840, left_stereo, nWidth, {nWidth, nHeight}, NPPI_INTER_CUBIC);
                    nppiRemap_8u_C1R(this->right_gray, {nWidth, nHeight}, nWidth, {0, 0, nWidth, nHeight}, pRxmap, 3840, pRymap, 3840, right_stereo, nWidth, {nWidth, nHeight}, NPPI_INTER_CUBIC);
                }
                ...
                if (this->frame >= this->sync_frame)
                {
                    ...
                    // LOG(INFO) << "id:" << this->id_front << ",gframe_dura:" << std::setprecision(20) << this->gframe_dura / 1000000000.0;
                    // LOG(INFO) << "id:" << this->id_front << std::setprecision(20) << ",ros time:" << ros::Time::now().toSec();
                    ...
                    {
                        LOG(WARNING) << "id:" << this->id_front << ",is drop frame:" << this->dropframe;
                        this->dropframe = false;
                        LOG(WARNING) << "timestamp exception,last_time:" << this->last_time_image.toSec()
                                     << ",current_time:" << std::setprecision(20) << this->time_image.toSec();
                    ...
                    {
                        LOG(ERROR) << "old time>new time,old:" << std::setprecision(20) << this->last_time_image.toSec()
                                   << ",new:" << std::setprecision(20) << this->time_image.toSec();
                        this->time_image = this->last_time_image + ros::Duration(0, this->pwm_period * 1000000);
                    }
                    // LOG(INFO) << "id:" << this->id_front << ",time_image:" << std::setprecision(20) << this->time_image.toSec();
        ...
        else
        {
            if (control_mode == 1)
            {
                LOG(WARNING) << "ros time:" << std::setprecision(20) << ros::Time::now().toSec() << ",cannot get image";
