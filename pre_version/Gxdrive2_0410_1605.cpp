...
void Gxdrive::depth_map()
{
    while (ros::ok())
    {
        if (this->frame > this->sync_frame + 4 && left_gray != NULL && right_gray != NULL)
        {
            ...
            vpiData_right->buffer.pitch.format = VPI_IMAGE_FORMAT_Y8;
            vpiData_right->buffer.pitch.numPlanes = 1;
            VPIPixelType pixType_right = vpiImageFormatGetPlanePixelType(VPI_IMAGE_FORMAT_U8, 0);
            VPIImagePlanePitchLinear &plane_right = vpiData_right->buffer.pitch.planes[0];
            plane_right.width = this->width;
            plane_right.height = this->height;
            plane_right.pitchBytes = this->width;
            plane_right.pixelType = pixType_right;

            memset(vpiData_left, 0, sizeof(*vpiData_left));
            vpiData_left->bufferType = VPI_IMAGE_BUFFER_HOST_PITCH_LINEAR;
            vpiData_left->buffer.pitch.format = VPI_IMAGE_FORMAT_Y8;
            vpiData_left->buffer.pitch.numPlanes = 1;
            VPIPixelType pixType_left = vpiImageFormatGetPlanePixelType(VPI_IMAGE_FORMAT_U8, 0);
            VPIImagePlanePitchLinear &plane_left = vpiData_left->buffer.pitch.planes[0];
            plane_left.width = this->width;
            plane_left.height = this->height;
            plane_left.pitchBytes = this->width;
            plane_left.pixelType = pixType_left;

            this->lock.lock();
            cudaMallocManaged(&vpiData_left->buffer.pitch.planes[0].data, this->width * this->height * 8);
            memcpy((u_char *)vpiData_left->buffer.pitch.planes[0].data, this->left_gray, this->width * this->height);
            // plane_left.data = this->left_gray;
            plane_right.data = this->right_gray;
            std::cout << (int)*this->left_gray << std::endl;
            std::cout << (int)*(this->left_gray + 1) << std::endl;
            std::cout << (int)*(this->left_gray + 2) << std::endl;
            std::cout << (int)*(this->left_gray + 3) << std::endl;
            ...
            CHECK_STATUS(vpiImageCreateWrapper(vpiData_left, nullptr, 0, &inLeft));
            CHECK_STATUS(vpiImageCreate(960, 768, VPI_IMAGE_FORMAT_Y8, 0, &inRight));

            // CHECK_STATUS(vpiImageCreateWrapper(vpiData_right, nullptr, VPI_BACKEND_CPU, &inRight));
            CHECK_STATUS(vpiSubmitConvertImageFormat(stream, VPI_BACKEND_CUDA, inLeft, inRight, NULL));
            std::cout << "b" << std::endl;
            ...
            this->lock.unlock();
            // std::cout << (int)*(u_char *)vpiData_left->buffer.pitch.planes[0].data << std::endl;
            std::cout << (int)*((u_char *)outPitch.planes[0].data + 1) << std::endl;
            std::cout << (int)*((u_char *)outPitch.planes[0].data + 2) << std::endl;
            std::cout << (int)*((u_char *)outPitch.planes[0].data + 3) << std::endl;
            std::cout << "b" << std::endl;
