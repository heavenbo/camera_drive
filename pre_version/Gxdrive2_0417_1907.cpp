    ...
    while (ros::ok())
    {
        ...
        if (status == GX_STATUS_SUCCESS)
        {
            ...
            if (this->g_frame_data.nStatus == 0)
            {
                ...
                // lock.lock();
                memcpy((u_char *)vpiLeft.data, this->left_gamma, nWidth * nHeight * 3);
                memcpy((u_char *)vpiRight.data, this->right_gamma, nWidth * nHeight * 3);
                // lock.unlock();
...
void Gxdrive::depth_map()
{
    while (ros::ok())
    {
        if (this->frame > this->sync_frame + 4 && left_gray != NULL && right_gray != NULL)
        {
            ...
            // vpiLeft.convertTo(vpiLeft, CV_16UC3);
            // vpiRight.convertTo(vpiRight, CV_16UC3);
            ...
            ROS_INFO("img data:%d", (int)*vpiLeft.data);
            ...
            lock.lock();
            CHECK_STATUS(vpiImageCreateWrapperOpenCVMat(vpiLeft, 0, &left));
            CHECK_STATUS(vpiImageCreateWrapperOpenCVMat(vpiRight, 0, &right));
            lock.unlock();
            // Create the image where the disparity map will be stored.
            CHECK_STATUS(vpiImageCreate(960, 768, VPI_IMAGE_FORMAT_S16, 0, &disparity));
            CHECK_STATUS(vpiImageCreate(960, 768, VPI_IMAGE_FORMAT_S8, 0, &stereoLeft));
            CHECK_STATUS(vpiImageCreate(960, 768, VPI_IMAGE_FORMAT_S8, 0, &stereoRight));
            CHECK_STATUS(vpiSubmitConvertImageFormat(stream, VPI_BACKEND_CUDA, left, stereoLeft, NULL));
            CHECK_STATUS(vpiSubmitConvertImageFormat(stream, VPI_BACKEND_CUDA, right, stereoRight, NULL));
            // Create the payload for Harris Corners Detector algorithm
            // CHECK_STATUS(vpiCreateStereoDisparityEstimator(backendType, 960, 768,
            //                                                VPI_IMAGE_FORMAT_NV12_ER, NULL, &stereo));
            ...
            // CHECK_STATUS(vpiSubmitStereoDisparityEstimator(stream, backendType, stereo, stereoLeft, stereoRight, disparity, NULL, &params));
            ...
            VPIImageData data;
            CHECK_STATUS(vpiImageLockData(stereoLeft, VPI_LOCK_READ, VPI_IMAGE_BUFFER_HOST_PITCH_LINEAR, &data));
            memcpy(reinterpret_cast<u_char *>(&this->stereo_disparity.data[0]), (u_char *)data.buffer.pitch.planes[0].data, width * height);
            CHECK_STATUS(vpiImageUnlock(stereoLeft));
