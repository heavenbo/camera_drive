...
private:                                                      // 参数
    ...
    uint64_t frame = 0;               // 帧数
    uint64_t gframe_img_dura_min = 0; // 与fpga实际时间戳的间隔最小值 ns
    uint64_t gframe_img_dura;         // 与fpga的时间差
    uint64_t gframe_img_dura_first;   // 第一次启动时间 debug
    uint64_t gframe_ros_dura_first;   // 第一次启动时间 debug
    uint64_t img_ros_dura_first;      // 第一次启动时间 debug
public:                               // 数据结构
    std::fstream text;
    ...
    ros::Duration last_tmp_inter;                     // 图像之间间隔
    ros::Duration duration_min = ros::Duration(1, 0); // 最小间隔
